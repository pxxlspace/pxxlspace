...
async function main() {
    ...
    if (command === "help" && args[0])
        return print(commandHelp(args[0]));
    if (!command || command === "help" || command === "--help" || command === "-h")
        return print(usage);
    if (isHelpRequest(args))
        return print(commandHelp(command));
    if (command === "login")
        return login(args);
    if (command === "logout") {
    ...
    if (command === "storage")
        return storage(client, args);
    if (command === "analytics")
        return analytics(client, args);
    if (command === "customer" || command === "customers")
        return customers(client, args);
    if (command === "invoice" || command === "invoices")
        return invoices(client, args);
    if (command === "billing")
        return billing(client, args);
...
async function storage(client, args) {
...
async function analytics(client, args) {
...
async function customers(client, args) {
    const command = args.shift() || "list";
    if (command === "list" || command === "ls") {
    ...
    if (command === "create") {
        const input = jsonFlag(args, "--data");
...
async function invoices(client, args) {
    const command = args.shift() || "list";
    ...
    if (command === "payment-url" || command === "payment") {
        const id = required(firstValueArg(args), "invoice id");
        const result = await spinner("Creating payment URL", () => client.getPaymentUrl(id, flagValue(args, "--currency"), teamId));
        if (wantsJSON(args))
            return printJSON(result);
...
async function billing(client, args) {
    const command = args.shift() || "list";
    const teamId = flagValue(args, "--team");
    if (command === "list" || command === "ls") {
...
function jsonFlag(args, name) {
    ...
    if (!raw)
        throw new Error(`${name} requires a JSON object`);
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
