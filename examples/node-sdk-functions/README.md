# Node SDK Functions Example

Copy these functions into a backend script, CI job, or internal tool when you want to automate Pxxl with JavaScript.

```bash
npm install
PXXL_API_KEY=pxxl_... npm run demo
```

The example includes:

- Deploying a local codebase with the SDK
- Uploading and listing CDN assets
- Searching domains and reading TLD prices
- Fetching teams, projects, deployments, usage, and stats
- Creating databases and pushing encrypted project env vars

Docs: https://docs.pxxl.app  
Deploy on Pxxl: https://pxxl.app
