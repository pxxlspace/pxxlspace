...
<AccordionGroup>
  ...
  <Accordion title="Database not visible inside a project">
    ...
    <Steps>
      <Step title="Confirm the database is active">
        Open [Database](/database/overview) and check that the database status shows active. A database that is still provisioning or in an error state cannot accept connections.
      </Step>
  ...
  <Accordion title="Container checklist">
    ...
    | Database is active | The database status in [Database](/database/overview) shows active, not provisioning or errored |
...
<Tip>
  For the complete database setup flow, including provisioning, connection string formats, and backups, see the [Database documentation](/database/overview).
</Tip>
